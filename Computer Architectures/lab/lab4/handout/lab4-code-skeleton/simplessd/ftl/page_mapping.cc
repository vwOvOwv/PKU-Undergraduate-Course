...
void PageMapping::readInternal(Request &req, uint64_t &tick) {
  ...
  if (mappingList != table.end()) { // if the mapping list is found
    // simulate DRAM (mapping table) access latency
    if (bRandomTweak) {
...
void PageMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  ...
  auto mappingList = table.find(req.lpn); // mapping list: <lpn, vector<pbn, ppn>>
  uint64_t beginAt;
  uint64_t finishedAt = tick;
  ...
  // simulate DRAM access latency. Read-modify-write the mapping table.
  if (sendToPAL) {
    if (bRandomTweak) {
      pDRAM->read(&(*mappingList), 8 * req.ioFlag.count(), tick);
