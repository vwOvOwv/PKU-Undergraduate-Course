...
class CaptioningRNN:
    ...
    def sample(self, features, max_length=30):
        ...
        # functions; you'll need to call rnn_step_forward in a loop.              #
        #                                                                         #
        ...
        x_in = np.ones(shape=(N, 1)) * self._start  # (N, 1)
        prev_h = h0 # (N, H)
        for t in range(max_length):
            x_in = x_in.astype(int)
            x_embed, _ = word_embedding_forward(x_in, W_embed)  # (N, 1, D)
            x_embed = np.squeeze(x_embed, axis=1) # (N, D)
            next_h, _ = rnn_step_forward(x_embed, prev_h, Wx, Wh, b)  # (N, H)
            next_h = next_h[:, np.newaxis, :] # (N, 1, H)
            y, _ = temporal_affine_forward(next_h, W_vocab, b_vocab)  # (N, 1, V)
            y = np.squeeze(y, axis=1)
            prob = np.exp(y) / np.sum(np.exp(y))  # (N, V)
            captions[:, t] = np.argmax(prob, axis=1) # (N, 1)
            x_in = captions[:, t].reshape(N, 1)  # (N, 1)
            prev_h = np.squeeze(next_h, axis=1)  # (N, H)
