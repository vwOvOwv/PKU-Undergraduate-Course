...
if __name__ == "__main__":
    ...
    y = np.zeros((len(epsilons), iter), dtype=np.float64)
    values = np.zeros((len(epsilons), n*2+1), dtype=np.float64)
    counts = np.zeros((len(epsilons), n*2+1), dtype=np.int64)
