...
def compute_iou(box, boxes, box_area, boxes_area):
    ...
    ## ---------------------------------------------------------- ##
    # print(box)
    box = box.reshape(-1, 1)    # to broadcast
    # print(boxes)
...
class MultiShapeDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        color = np.asarray([random.randint(0, 255) for _ in range(3)])
        img[:, :, 0] = color[0]
        img[:, :, 1] = color[1]
        img[:, :, 2] = color[2]
        ## ------------------------------------------- ##
        ...
        obj_param = np.zeros((num_objs, 4))     # obj_param: [y, x, size, shape_id]
        obj_ids = np.zeros((num_objs)) 
        
        for i in range(num_objs):
            obj_param[i,:] = self._draw_one_shape_on_mask(masks[i, :], random.randint(1, 3))
            obj_ids[i] = obj_param[i, 3] # shape_id
        
        # print("obj_param", obj_param)
        ...
        for i in range(0, masks.shape[0] - 1):
            for j in range(i + 1, masks.shape[0]):
                if np.sum(masks[i] * masks[j]) > 0: # have occlusion
                    masks[j] -= masks[i] * masks[j]
        ## --------------------------------------------------- ##
