...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(z)
    cache = (x, prev_h, Wx, Wh, b, 1 - next_h**2)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, local_dz = cache
    dz = dnext_h * local_dz
    dx, dWx, db = affine_backward(dz, (x, Wx, b))
    dprev_h, dWh, _ = affine_backward(dz, (prev_h, Wh, b))
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        if t == 0:
            h = h_t[:, np.newaxis, :]    # (N, 1, H)
        else:
            h = np.concatenate((h, h_t[:, np.newaxis, :]), axis=1)
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
        cache_t = cache[t]  # (x, prev_h, Wx, Wh, b, local_dz=dh/dz)
        dnext_h = dh[:, t, :]
        
        # h(t) has two sources of gradient, one is from t + 1, another is from t
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h + dprev_h, cache_t)
        ...
        if t == T - 1:
            ...
            dx = np.concatenate((dx, dx_t[:, np.newaxis, :]), axis=1)
            dWx += dWx_t
            dWh += dWh_t
            db += db_t
