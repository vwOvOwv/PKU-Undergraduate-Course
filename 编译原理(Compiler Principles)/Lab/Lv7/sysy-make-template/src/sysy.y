...
Stmt
  ...
  | RETURN Exp ';' {
    ...
    ast->exp = unique_ptr<ExpAST>(static_cast<ExpAST*>($2));
    $$ = ast;
  }
  | RETURN ';' {
    auto ast = new ReturnStmtAST();
...
Exp
  : LOrExp {
    auto ast = new ExpAST();
    ast->lor_exp = unique_ptr<LOrExpAST>(static_cast<LOrExpAST*>($1));
    $$ = ast;
  }
  ;
...
PrimaryExp
  ...
  | Number {
    auto ast = new PrimaryExpAST();
    ast->number = make_unique<NumberAST>();
    ast->number->number = $1;
    $$ = ast;
  }
  ;
...
MulExp
  : UnaryExp {
    ...
    ast->unary_exp = unique_ptr<UnaryExpAST>(static_cast<UnaryExpAST*>($1));
    $$ = ast;
  }
  | MulExp '*' UnaryExp {
    auto ast = new MulExpAST();
    ast->left = unique_ptr<MulExpAST>(static_cast<MulExpAST*>($1));
    ast->op = "*"; // 直接赋值，不要去读 $2
    ast->right = unique_ptr<UnaryExpAST>(static_cast<UnaryExpAST*>($3)); // 注意这里是 $3
    $$ = ast;
  }
  | MulExp '/' UnaryExp {
    auto ast = new MulExpAST();
    ast->left = unique_ptr<MulExpAST>(static_cast<MulExpAST*>($1));
    ast->op = "/";
    ast->right = unique_ptr<UnaryExpAST>(static_cast<UnaryExpAST*>($3));
    $$ = ast;
  }
  | MulExp '%' UnaryExp {
    auto ast = new MulExpAST();
...
AddExp
  : MulExp {
    ...
    ast->mul_exp = unique_ptr<MulExpAST>(static_cast<MulExpAST*>($1));
    $$ = ast;
  }
  | AddExp '+' MulExp {
    auto ast = new AddExpAST();
    ast->left = unique_ptr<AddExpAST>(static_cast<AddExpAST*>($1));
    ast->op = "+";
    ast->right = unique_ptr<MulExpAST>(static_cast<MulExpAST*>($3));
    $$ = ast;
  }
  | AddExp '-' MulExp {
    auto ast = new AddExpAST();
...
LOrExp
  : LAndExp {
    ...
    ast->land_exp = unique_ptr<LAndExpAST>(static_cast<LAndExpAST*>($1));
    $$ = ast;
  }
  | LOrExp OR LAndExp {
    auto ast = new LOrExpAST();
    ast->left = unique_ptr<LOrExpAST>(static_cast<LOrExpAST*>($1));
    ast->op = "||";
    ast->right = unique_ptr<LAndExpAST>(static_cast<LAndExpAST*>($3));
    $$ = ast;
  }
  ;
...
LAndExp
  : EqExp {
    ...
    ast->eq_exp = unique_ptr<EqExpAST>(static_cast<EqExpAST*>($1));
    $$ = ast;
  }
  | LAndExp AND EqExp {
    auto ast = new LAndExpAST();
    ast->left = unique_ptr<LAndExpAST>(static_cast<LAndExpAST*>($1));
    ast->op = "&&";
    ast->right = unique_ptr<EqExpAST>(static_cast<EqExpAST*>($3));
    $$ = ast;
  }
  ;
...
EqExp
  : RelExp {
    ...
    ast->rel_exp = unique_ptr<RelExpAST>(static_cast<RelExpAST*>($1));
    $$ = ast;
  }
  | EqExp EQ RelExp {
    auto ast = new EqExpAST();
    ast->left = unique_ptr<EqExpAST>(static_cast<EqExpAST*>($1));
    ast->op = "==";
    ast->right = unique_ptr<RelExpAST>(static_cast<RelExpAST*>($3));
    $$ = ast;
  }
  | EqExp NE RelExp {
    auto ast = new EqExpAST();
...
RelExp
  : AddExp {
    ...
    ast->add_exp = unique_ptr<AddExpAST>(static_cast<AddExpAST*>($1));
    $$ = ast;
  }
  | RelExp '<' AddExp {
    auto ast = new RelExpAST();
    ast->left = unique_ptr<RelExpAST>(static_cast<RelExpAST*>($1));
    ast->op = "<";
    ast->right = unique_ptr<AddExpAST>(static_cast<AddExpAST*>($3));
    $$ = ast;
  }
  | RelExp '>' AddExp {
    auto ast = new RelExpAST();
